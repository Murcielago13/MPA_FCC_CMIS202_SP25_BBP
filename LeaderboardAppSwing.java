...
public class LeaderboardAppSwing {
    ...
    private static final String INSTRUCTION_TEXT = "Welcome to the Leaderboard System!\n\n" +
            ...
            "   - Player Name: Type a new name or select an existing one. The 'Delete This Player's Data' button next to this input field removes all scores for the currently entered/selected player (confirmation required).\n" +
    ...
    private void createAndShowGUI() {
        ...
        frame.setSize(950, 550); // Set the initial size of the window.
        ...
        inputControlsPanel.add(deletePlayerByNameButton); // Button to delete all data for the player in playerNameInputComboBox.
        ...
        // Note: deletePlayerByNameButton was intentionally moved to inputControlsPanel in previous design iterations.
        // Its functionality is tied to the playerNameInputComboBox.
        JLabel spacerX = new JLabel(); // Spacer.
        ...
        deletePlayerScoreButton.addActionListener(e -> {
            ...
            for (ScoreEntry entryToDelete : selectedEntries) {
                ...
                if (scoreMap.remove(getCompositeKey(entryToDelete.getName(), entryToDelete.getGameName())) != null) {
                    ...
                    scoreTree.delete(entryToDelete.getName()); // This deletes based on name, might need refinement if player has multiple scores.
                                                               // However, scoreMap is the source of truth, tree is rebuilt or updated.
                                                               // For BST, it typically stores unique keys. If player names map to one entry, this works.
                                                               // Given scoreMap uses composite keys, player name alone is not unique there.
                                                               // The BST might be storing one representative entry or needs specific handling for multiple game scores per player.
                                                               // Current BST insertRec updates data if name matches, so one entry per player.
    ...
    private boolean performPlayerDataDeletion(Set<String> playerNamesToProcess) {
        ...
        for (String pName : playerNamesToProcess) {
            ...
            if (scoresRemovedForThisPlayer) {
                ...
                // After removing all scores for pName from scoreMap, remove pName from uniquePlayerNames.
                // This ensures that if a player has no scores, their name doesn't linger in the suggestion list.
                uniquePlayerNames.remove(pName);
            } else {
                // If no scores were found in scoreMap for this player (e.g., name was only typed but no score submitted),
                // still attempt to remove from uniquePlayerNames, as it might be there if it was previously added
                // (e.g., via playerNameInputComboBox directly if that logic existed, or from a previous score).
                // The remove() method on HashSet handles non-existent elements gracefully.
                uniquePlayerNames.remove(pName);
            }
        }
        return dataActuallyChanged; // Return true if any player's scores were actually removed from the map.
    }
    ...
    private void updatePlayerNameInputComboBoxModel() {
        ...
        if (model.getSize() == 0) {
            ...
            // Otherwise, set the editor text back to what it was.
    ...
    private void saveScores() {
        ...
        // Sort this list based on ScoreEntry's compareTo, for consistent file output.
        // This sorting primarily helps if the file is manually inspected or used by another process.
        // The application itself relies on sorting during display (refreshLeaderboard).
        ...
        if (file.exists()) {
            ...
            try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    String[] parts = line.split(",", 4); // Limit to 4 parts to handle commas in names/games if any (though current design assumes no such commas).
                    if (parts.length == 4) {
                        try {
    ...
    private void merge(ArrayList<ScoreEntry> list, int left, int mid, int right) {
        ...
        while (i < leftList.size() && j < rightList.size()) {
            // ScoreEntry.compareTo sorts by Score (DESC), Date (DESC), Player Name (ASC).
            // A non-positive result means leftList.get(i) should come before or is equal to rightList.get(j).
     ...
     * This data structure is maintained alongside `scoreMap` but `scoreMap` is
     * often the primary source for current data. This list can be useful for
     * sequential access or specific list operations not directly tied to key-based lookups.
     */
     ...
     * If multiple scores exist for the same player (e.g., in different games),
     * this BST (as implemented with `insertRec` updating data on name match)
     * would effectively store/update only one ScoreEntry per unique player name.
     * This means it's primarily useful if operations are player-name-centric
     * and a single representative entry per player is desired in the tree.
     * It is maintained in conjunction with `scoreMap`.
     */
    static class ScoreBST {
        ...
        private Node insertRec(Node currentRoot, ScoreEntry entry) {
            // If the current subtree is empty, create a new node and make it the root.
            if (currentRoot == null) {
                return new Node(entry);
            }

            // Compare the player name of the entry to insert with the name in the current node.
            int nameComparison = entry.getName().compareTo(currentRoot.data.getName());

            if (nameComparison < 0) { // If new entry's name is less, go to the left subtree.
                currentRoot.left = insertRec(currentRoot.left, entry);
            } else if (nameComparison > 0) { // If new entry's name is greater, go to the right subtree.
                currentRoot.right = insertRec(currentRoot.right, entry);
            } else {
                // If names are the same, update the data in the current node.
                // This means the BST effectively stores one entry per player name, the latest one inserted.
                currentRoot.data = entry;
            }
            return currentRoot; // Return the (possibly modified) root of the subtree.
        }
        ...
        public ScoreEntry findByName(String name) {
            ...
            while (current != null) {
                ...
                if (cmp == 0) return current.data; // Found the entry.
        ...
        private Node deleteRec(Node currentRoot, String name) {
            if (currentRoot == null) return null; // Name not found in this path.

            int cmp = name.compareTo(currentRoot.data.getName());
            if (cmp < 0) { // Name to delete is smaller, go left.
                currentRoot.left = deleteRec(currentRoot.left, name);
            } else if (cmp > 0) { // Name to delete is larger, go right.
